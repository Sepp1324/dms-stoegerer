...
if [ "$missing" -ne 0 ] || [ "$mismatch" -ne 0 ] || [ "$arch_missing" -ne 0 ] || [ "$arch_bad" -ne 0 ]; then
  record_drill_status failed \
    "Restore-Drill FEHLGESCHLAGEN: ${missing} fehlende Dateien, ${mismatch} Hash-Abweichungen, ${arch_missing} fehlende + ${arch_bad} defekte Archive (von ${checked}/${arch_checked} geprüft)"
  echo "FEHLER: Backup nicht vollständig wiederherstellbar – ${missing} fehlend, ${mismatch} Hash-Mismatch, ${arch_missing} Archiv fehlt, ${arch_bad} Archiv defekt." >&2
  exit 1
fi

record_drill_status success \
  "Restore-Drill erfolgreich: ${DOCS} Dokumente, ${FILES} Dateien, ${checked} Version-Dateien (Hash) + ${arch_checked} Archive (Größe/PDF-Magic) verifiziert"
MONITOR_OK=1
