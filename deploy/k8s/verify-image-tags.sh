...
for image in dms-backend dms-frontend; do
  ...
  if docker manifest inspect "$ref" >/dev/null 2>&1 || docker manifest inspect --insecure "$ref" >/dev/null 2>&1; then
    echo "OK:     ${image}:${tag} vorhanden."
  else
    echo "FEHLER: ${image}:${tag} NICHT per Docker manifest inspect abrufbar – apply -k würde ErrImagePull auslösen."
    fail=1
  fi
done
